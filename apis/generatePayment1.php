...
if (isset($_POST['postdata']) && isset($_POST['packageDetails']) && isset($_POST['uFlag'])) {
    ...
    $pkgDetailsArray   = json_decode($packageDetails, true);
    $uFlagArray   = json_decode($uFlag, true);
    ...
    if($uFlagArray['uFlag'] == 1 && $uFlagArray['paymentId'] != null){
        $payId = $uFlagArray['paymentId'];
        $packageUpdate =1;
        if(!empty($uFlagArray['recieved'])){
            $recieved = $uFlagArray['recieved'];
        }
        $u = 1;
    ...
    if($pkgDetailsArray['flag'] == 1 && !empty($pkgDetailsArray['packageId'])){
        $isPackage = 1;
        if($packageUpdate != 1){
        package($pkgDetailsArray['packageId'],$patientId,$pkgDetailsArray['packageCost'],1,$branchId,$userId,$pkgDetailsArray['packageDuration']);
        }
    }
    $packageIdopd    = !empty($pkgDetailsArray['packageId'])? $pkgDetailsArray['packageId']:'NULL';
